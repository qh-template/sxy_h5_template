...
module.exports = {
  ...
  plugins: [
    new webpack.optimize.CommonsChunkPlugin({
      ...
      filename: 'static/js/[name].[chunkhash].js', // 路径为dist下（根据output.path）
      minChunks: 2
    }),
    ...
    // }),
    new ExtractTextPlugin('static/css/[name].[contenthash].css'), // 把css提取出来成为文件;路径为dist下（根据output.path）
    new PurifyCssPlugin({
      ...
      paths: glob.sync(path.join(__dirname, './*.html')) // glob模块允许你使用 *等符号, 来写一个glob规则
    }),
